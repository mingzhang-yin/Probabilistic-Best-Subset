...
def data_gen(N ,P, rho, seed=1):
    ...
    sigma = np.sqrt(np.matmul(np.matmul(beta_true, V),beta_true) / args.snr) 
...
if __name__ == '__main__':
    ...
    # General info
    parser.add_argument('--name', '-n', default='exp2_u2gvb', help='exp,model name')
    parser.add_argument('--grad', '-g', type=str, default='u2g', \
                        help='reinforce, arm, u2g')
    parser.add_argument('--path', default='K/', help='The path results to be saved.')
    ...
    parser.add_argument('--lr', '-l', type=float, default=0.04, help='base lr')
    ...
    for K in np.arange(1,30,3):
        path = os.path.join(args.path, f'{args.name}',f'{K}', f'{args.lmbd}')
        if not os.path.exists(path):
            os.makedirs(path) 
        for count, seed in enumerate(np.arange(2010,2020,1)): 
            X,Y,COV,beta_true = data_gen(args.N,args.P,args.rho,seed=seed)     
            z_true = (beta_true>0)  
            ...
            u_noise = tf.random_uniform(shape=[K,args.P],maxval=1.0)  #K*P
            ...
            if args.grad == 'arm':
                ...
                phi_tile = tf.tile(phi[None,:], [K,1]) #K*P
            ...
            #Calculate metric
            sigma = np.sqrt(np.matmul(np.matmul(beta_true, COV),beta_true) / args.snr) 
            ...
            all_ = [prec, rec, F1, nonzero, RR, RTE_g, PVE, corr, incorr, n_fail, RTE_ols, duration, args]
