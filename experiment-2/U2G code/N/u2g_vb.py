...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', default='N/', help='The path results to be saved.')
    ...
    for N in np.arange(60,141,8):
        path = os.path.join(args.path, f'{args.name}',f'{N}', f'{np.round(args.lmbd,2)}')
        if not os.path.exists(path):
            os.makedirs(path) 
        for count, seed in enumerate(np.arange(2000,2020,1)): 
            X,Y,COV,beta_true = data_gen(N,args.P,args.rho,seed=seed)     
            z_true = (beta_true>0)  
            ...
            x = tf.placeholder(tf.float32,[N, args.P],name='data_x')  
            ...
            def fun(E):
                logit_pi = np.float32(- args.lmbd/(2 * args.sigma**2))     
