...
if __name__ == '__main__':
    ...
    # General info
    parser.add_argument('--name', '-n', default='exp1_u2gvb', help='exp,model name')
    parser.add_argument('--grad', '-g', type=str, default='u2g', \
                        help='reinforce, arm, u2g')
    parser.add_argument('--path', default='RHO/', help='The path results to be saved.')
        
    # Algorithm parameters
    parser.add_argument('--lmbd', type=float, default=30.0, \
    ...
    for rho in np.arange(0,1,0.1):
        path = os.path.join(args.path, f'{args.name}',f'{np.round(rho,2)}', f'{args.lmbd}')
        if not os.path.exists(path):
            os.makedirs(path) 
        for count, seed in enumerate(np.arange(2000,2020,1)): 
            X,Y,COV,beta_true = data_gen(args.N,args.P,rho,seed=seed)     
            z_true = (beta_true>0)  
